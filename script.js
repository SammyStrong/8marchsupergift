...
function initFullGalaxy() {
    ...
    // --- ЖИРНЫЕ КОЛЬЦА (Сделал больше слоев и плотность) ---
    const createRing = (radius, width, count, size, color) => {
        const pos = [];
        for(let i=0; i<count; i++){
            const a = Math.random() * Math.PI * 2;
            const r = radius + (Math.random() - 0.5) * width;
            // Увеличил разброс по Y, чтобы кольца были объемными
    ...
    group.add(createRing(800, 100, 15000, 10, 0xffffff)); // Внутреннее
    group.add(createRing(1000, 150, 20000, 12, 0xffffff)); // Среднее
    group.add(createRing(1300, 250, 25000, 14, 0xffffff)); // Внешнее

    // --- МНОГО КАРТИНОК (100 ШТУК) ---
    ...
    for(let i=0; i<100; i++){
        loader.load(imgs[i%8], (t) => {
            ...
            const a = Math.random()*Math.PI*2;
            const r = 850 + Math.random() * 1500; // Разнес дальше
    ...
    function anim() {
        ...
        phs.forEach(m => {
            ...
            m.lookAt(camera.position); // Фото всегда повернуты к Аише
        });
